...
main(int argc, char** argv)
{
  ...
  if(basic_config.at("ConfigFiles").size() != basic_config.at("DTSolutions").size())
  {
    throw std::length_error(
      "Error in configuration input file: *ConfigFiles* dimension does \
       not match *OutputFilesDT* dimension. If no DT solution is present \
       please use the dt_solver"
    );
  }
  ...
  for(size_t i=0; i < basic_config.at("ConfigFiles").size(); ++i)
  {
    ...
    for(const auto& sol_: new_rg_sols)
    {
      ...
      {
        const std::string err = "ERROR: RT-RG selected unavailable Edge \
                                 resources wrt the given solution";
      ...
      for(
        size_t i = 0; 
        i < std::max(selected_vms_by_cl_.size(), curr_sel_vms_by_cl.size()); 
        ++i
      )
      {
        ...
        {
          if(
            selected_vms_by_cl_.at(i).first && 
            (selected_vms_by_cl_.at(i).second != curr_sel_vms_by_cl.at(i).second)
          )
          {
            const std::string err = "ERROR: RT-RG selected different VM at \
                                     an already chosen layer in the given \
                                     solution";
            throw std::logic_error(err);
          }
        }
    ...
    for(const auto& ls: ls_vec)
    {
      ...
      if(curr_sel_res.get_selected_edge() < selected_resources_.get_selected_edge())
      {
        const std::string err = "ERROR: RT-LS selected different Edge \
                                 resources wrt the given solution";
        throw std::logic_error(err);
      }
      ...
      )
      {
        if(curr_sel_vms_by_cl.at(i).first)
        {
          ...
          {
            const std::string err = "ERROR: RT-LS selected different VM at \
                                     an already chosen layer in the given \
                                     solution";
            throw std::logic_error(err);
          }
  ...
  Logger::Info("Mask-Detection test completed");
}
