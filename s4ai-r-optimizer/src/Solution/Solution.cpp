...
)
{
  ...
  for(const auto& [comp, comp_data] : configuration_file.at("components").items())
  {
    ...
    dbg_msg << "  Component " << comp << " (index " << comp_idx << ")";
    Logger::Debug(dbg_msg.str());
    ...
    dbg_msg.str("");
    dbg_msg << "    deployment " << key;
    Logger::Debug(dbg_msg.str());

    // get info about the selected deployment
    nl::json depl_data = it.value();
    for (const auto& [part, part_data] : depl_data.items())
    {
      if(part == "response_time")
      {
        std::vector<TimeType> times = part_data.get<std::vector<TimeType>>();
        time_perfs.comp_perfs[comp_idx] = times[0];
        continue;
      }

      dbg_msg.str("");
      dbg_msg << "      partition " << part;
      Logger::Debug(dbg_msg.str());
      ...
      for(const auto& [cl, cl_data] : part_data.items())
      {
        ...
        for(const auto& [res, res_data] : cl_data.items())
        {
          ...
          dbg_msg.str("");
          dbg_msg << "        res " << res << " (type " << res_type_idx << ")";
          Logger::Debug(dbg_msg.str());
          ...
          else // Edge or VM
          {
            ...
            {
              selected_resources.selected_vms_by_cl[
                cl_name_to_idx[
          ...
          solution_data.used_resources[comp_idx].emplace_back(
            part_idx, res_type_idx, res_idx
          );
        }
      }
    }
...
Solution::to_json(const System& system) const
{
  ...
  for(std::size_t i = 0; i < components.size(); ++i)
  {
    ...
    // get deployments
    const auto& deployments = components[i].get_deployments();
    int found_deployment = -1;
    //get partitions
    ...
    for(size_t j=0; j<solution_data.used_resources[i].size(); ++j)
    {
      ...
      // get partition deployment
      std::string d_name = "X";
      for(
        size_t d_idx = 0; 
        d_idx < deployments.size() && found_deployment < 0; 
        ++d_idx
      )
      {
        const auto& p_indices = deployments[d_idx].get_partition_indices();
        for(size_t p_idx : p_indices)
          if(p_idx == part_idx)
            found_deployment = d_idx;
      }
      if(found_deployment >= 0)
        d_name = deployments[found_deployment].get_name();
      ...
      bool use_meanTime = (j < solution_data.used_resources[i].size() - 1);
      const TimeType t = time_perfs.compute_partition_perf(
        i, part_idx, res_type_idx, res_idx,
...
Solution::print_solution(const System& system, const std::string& path) const
{
  ...
  if(!outputsol)   // file couldn't be opened
  {
    ...
    throw std::runtime_error(
      "In *Solution::print_solution() *" + 
      path + 
      "* cannot be opened. Make sure that all the folders in path exist!"
    );
  }
...
void
Solution::print_unfeasible_solution(
  const System& system, const std::string& path
)
{
  // extract system data
  const auto& system_data = system.get_system_data();

  // generate json dict with only feasibility and the current workload
  nl::json jsolution;
  jsolution["Lambda"] = system_data.get_lambda();
  jsolution["feasible"] = false;
  ...
  if(!outputsol)   // file couldn't be opened
  {
    Logger::Error("File *" + path + "* cannot be opened");
    throw std::runtime_error(
      "In *Solution::print_unfeasible_solution() *" + 
      path + 
      "* cannot be opened. Make sure that all the folders in path exist!"
    );
  }
  ...
  outputsol << std::setw(4) << jsolution << std::endl;
  outputsol.close();
  Logger::Info("Unfeasible solution saved at: " + path);
}
...
bool
Solution::check_QoS_constraints(const System& system)
{
  Logger::Info(
...
Solution::objective_function(const System& system, const LocalInfo& local_info)
{
  ...
  this->total_cost = 0.0;
  std::vector<std::vector<bool>> res_cost_alredy_computed(
  ...
  for(
    ...
    res_type_idx < ResIdxFromType(ResourceType::Count); 
    ++res_type_idx
  )
  {
    ...
    res_cost_alredy_computed[res_type_idx].resize(
      all_resources.get_number_resources(res_type_idx)
    );
  }

  for(
    size_t comp_idx = 0; 
    comp_idx < solution_data.used_resources.size(); 
    ++comp_idx
  )
  {
    for(
      const auto& [
        part_idx, res_type_idx, res_idx
      ] : solution_data.used_resources[comp_idx]
    )
    {
