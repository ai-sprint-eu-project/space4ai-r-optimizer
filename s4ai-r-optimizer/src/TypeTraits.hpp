...
/** MeanTimeType needed to store the average execution time of a given 
* component partition job on the given resource; indexed by 
* [comp_idx][type_idx][part_idx][res_idx].
*/
using MeanTimeType =
  std::vector<std::vector<std::vector<std::vector<TimeType>>>>;
