...
*
*   Class to store the data structures that define the System configuration 
*   described in the json configuration file except for the performance 
*   section.
*/
class SystemData
{
  ...
  public:
    ...
    *
    *   \param configuration_file nl::json object extracted from .json 
    *                             system_file
    */
    void
    read_json(
  ...
  private:
    ...
    *
    *   \param components_json nl::json object extracted from the .json 
    *                          configuration file containing the description 
    *                          of the components
    */
    void 
    ...
    *
    *   \param resources_json nl::json object extracted from the .json 
    *                          configuration file containing the description 
    *                          of the resources
    */
    ...
    *
    *   \param comp_matrix_json nl::json object extracted from the .json 
    *                           configuration file containing the description 
    ...
    */
    ...
    /** Method to initialize the network technology defined in 
    *   NetworkTechnology.hpp.
    *
    *   \param network_technology_json nl::json object extracted from the .json 
    ...
    *
    *   \param local_constraints_json nl::json object extracted from the .json 
    *                                 configuration file containing the 
    *                                 description of the local constraints
    */
    void 
    ...
    *
    *   \param global_constraints_json nl::json object extracted from the .json 
    *                                  configuration file containing the 
    *                                  description of the global constraints
    */
    void 
    initialize_global_constraints(const nl::json& global_constraints_json);

    /** Method to find the order of the Partition in the .json 
    *   configuration_file and assign to them increasing indexes.
    ...
    *
    *   \param parts_json nl::json object extracted from the .json 
    *                     configuration file, containing the partitions of a 
    *                     specific Component
    *   \return a std::map that associates an index to each Partition 
    ...
    *   Actually it is the same to the one stored in the dag, but it makes
    *   sense to have it also here.
    */
    ...
    */
    ...
    /** Map to obtain a Partition's index in the Component.partitions vector 
    *   knowing its name and its Component name: the key is 
    *   comp_name+part_name.
    */
    std::unordered_map<std::string, std::size_t>
    part_name_to_part_idx;

    /** Object to store the computational layers (defined in Resources.hpp) 
    ...
    *
    *   cl_name_to_idx[i] store the maps from layers containing resources out 
    *   of type i (0,1 or 2 i.e. ResourceType::Edge, ResourceType::VM or
    ...
    */
    ...
    /** Map to obtain a Resource's type and index in 
    *   AllResources.edge_resources or AllResources.VM_resources or 
    *   AllResources.Faas_resources, depending on its type, knowing its name.
    */
    ...
    /** Vector storing all the network domains (defined in 
    *   NetworkTechnology.hpp) 
    */
    std::vector<NetworkDomain> network_domains;

    /** Map to obtain the indexes of the network domains (in the 
    *   SystemData.network_domains vector) that contain a certain 
    *   ComputationalLayer (identified by its name)
    */
    std::unordered_map<std::string, std::vector<std::size_t>> 
    cl_to_network_domains;
    ...
    /** Map to obtain a GlobalConstraint's index in the 
    *   SystemData.global_constraints vector knowing its name (that 
    *   corresponds to the path's name)
    */
...
(const nl::json& faas_json)
{
  ...
  for(const auto& [key, data] : faas_json.items())
  {
    ...
    this->cls[ResIdxFromType(ResourceType::Faas)].emplace_back(
      key, ResourceType::Faas  // only one computational layer for faas!
    );
    ...
    for(const auto& [res_name, res_data] : data.items())
    {
      ...
      this->cls[ResIdxFromType(ResourceType::Faas)].back().add_resource(
        res_idx++
      );
      this->all_resources.add_resource(std::move(res));
    }
