...
{
  if(
    configuration_file.contains("DirectedAcyclicGraph") && 
    configuration_file.contains("Components")
  )
  {
    Logger::Info("Reading DAG...");
    this->dag.read_from_file(
      configuration_file.at("DirectedAcyclicGraph"), 
      configuration_file.at("Components")
    );
  ...
  {
    const std::string err_msg = "Cannot find DirectedAcyclicGraph or \
  ...
  if(configuration_file.contains("EdgeResources"))
  {
    ...
    this->initialize_resources<ResourceType::Edge>(
      configuration_file.at("EdgeResources")
    );
    Logger::Info("Done!");
  ...
  if(configuration_file.contains("CloudResources"))
  {
    ...
    this->initialize_resources<ResourceType::VM>(
      configuration_file.at("CloudResources")
    );
    Logger::Info("Done!");
    at_least_one_resource_type = true;
  }
  ...
  if(configuration_file.contains("FaaSResources"))
  {
    ...
    this->initialize_resources<ResourceType::Faas>(
      configuration_file.at("FaaSResources")
    );
    Logger::Info("Done!");
    at_least_one_resource_type = true;
  }
  ...
  // read components after the resources so I can resize memory field of 
  // partitions without pain...
  ...
  if(configuration_file.contains("CompatibilityMatrix"))
  {
    ...
    this->initialize_compatibility_matrix(
      configuration_file.at("CompatibilityMatrix")
    );
    Logger::Info("Done!");
  }
  else
  ...
  if(configuration_file.contains("LocalConstraints"))
  {
    ...
    this->initialize_local_constraints(
      configuration_file.at("LocalConstraints")
    );
    Logger::Info("Done!");
  }
  else
  {
    Logger::Info(
      "No Local Constraints provided in json file. Initializing them with +inf"
    );
  ...
  if(configuration_file.contains("GlobalConstraints"))
  {
    ...
    this->initialize_global_constraints(
      configuration_file.at("GlobalConstraints")
    );
    Logger::Info("Done!");
  }
  else
...
{
  const size_t num_edge = all_resources.get_number_resources(
    ResIdxFromType(ResourceType::Edge)
  );
  const size_t num_vms = all_resources.get_number_resources(
    ResIdxFromType(ResourceType::VM)
  );
  const size_t num_faas = all_resources.get_number_resources(
    ResIdxFromType(ResourceType::Faas)
  );
  ...
  for(const auto& [comp_idx, comp] : idx_to_comp_name)
  {
    ...
    // lambda^i in the paper (formula (1))
    LoadType comp_lambda = std::numeric_limits<LoadType>::quiet_NaN(); 
    ...
    {
      debug_message = "**** Initializing deployment " + dep + 
                      " of component " + comp;
      Logger::Debug(debug_message);
      ...
      for(const auto& [idx, part] : ordered_parts)
      {
        ...
        debug_message = "****** Initializing partition " + part + 
                        " of component " + comp;
        Logger::Debug(debug_message);
        ...
        early_exit_probability = data.at(
          "early_exit_probability"
        ).get<ProbType>(); // early exit probability for the next partition
        ...
        if(idx == ordered_parts.size() - 1) // last partition
        {
          for(size_t i = 0; i < data.at("next").size(); ++i)
          {
            ...
            if(comp_next_name == "")
            {
              Logger::Error("Next component name is empty");
              throw std::runtime_error("Next component name is empty");
            }

            next_data_sizes.emplace(
              comp_name_to_idx.at(comp_next_name), data_size_next
            );
          }
        }
        else
        {
          ...
          if(part_next_name == "")
          {
            Logger::Error("Next partition name is empty");
            throw std::runtime_error("Next partition name is empty");
          }
...
SystemData::find_order_parts(const nl::json& parts_json) const
{
  ...
  for(const auto& [part_name, part_data] : parts_json.items())
  {
    ...
    const auto next_parts_json = part_data.at(
      "next"
    ...
    // if next_part is not a component
    if(comp_name_to_idx.count(next_parts_json.at(0)) == 0) 
    {
  ...
  for(std::size_t i = 0; i < all_parts.size(); ++i)
  {
    if(all_parts.count(root_part) == 0)
    {
      throw std::logic_error(
        "Error in find_order_parts: " + root_part + " is not a partition"
      );
    }

    idx_to_part_name.emplace(i, root_part);

    if(parts_json.at(root_part).at("next").size()) 
    {
      // size = 0 for last partition of last component
...
SystemData::initialize_compatibility_matrix(const nl::json& comp_matrix_json)
{
  ...
  for(const auto& [idx, comp] : idx_to_comp_name)
  {
    ...
    Logger::Debug(debug_message);
    // resize the second level of the compatibility matrix with the number 
    // of resource types
    ...
    const auto& partitions = components[
      comp_name_to_idx.at(comp)
    ].get_partitions();

    for(std::size_t i = 0; i < comp_temp_matrix.size(); ++i)
    {
      // resize the third level of the compatibility matrix with the number 
      // of partitions in the current component
      ...
      {
        // resize the fourth level of the compatibility matrix with the 
        // number of resources of type i
    ...
    for(const auto& [part, part_data] : comp_data.items())
    {
      ...
      for(const auto& res_and_mem : part_data)
      {
        ...
        comp_temp_matrix[ResIdxFromType(res_info.first)]
                        [p_idx]
                        [res_info.second] = 1;
        partition_p_idx.set_memory(
          memory.get<DataType>(), 
          ResIdxFromType(res_info.first), 
          res_info.second
        ); // set memory
      }
    }
...
SystemData::initialize_local_constraints(const nl::json& local_constraints_json)
{
  ...
  if(!local_constraints_json.empty())
  {
    ...
    {
      this->local_constraints.at(comp_name_to_idx.at(comp)) = LocalConstraint(
        comp_name_to_idx.at(comp), 
        data.at("local_res_time").get<TimeType>()
      );
    }
  }
}
...
void
SystemData::initialize_global_constraints(
  const nl::json& global_constraints_json
)
{
  std::string debug_message;
