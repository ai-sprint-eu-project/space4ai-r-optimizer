  ...
  size_t comp_idx, size_t part_idx, size_t res_idx)
{
  ...
  else if(model == "AMLFAAS" || model == "MLLIBfaas")
  {
    return std::make_unique<FaasaMLLibraryPE>(
        model, false, perf_json.at("demandWarm").get<TimeType>(), 
        perf_json.at("demandCold").get<TimeType>(),
        perf_json.at("regressor_file").get<std::string>()
      );
  }
  else if(model == "AMLCORE" || model == "CoreBasedPredictor")
  {
    return std::make_unique<CoreBasedaMLLibraryPE>(
        model, false, perf_json.at("regressor_file").get<std::string>()
      );
  }
  else if(model == "AMLTHROUGHPUT" || model == "LambdaBasedPredictor")
  {
    return std::make_unique<LambdaBasedaMLLibraryPE>(
        model, false, perf_json.at("regressor_file").get<std::string>()
      );
  }
